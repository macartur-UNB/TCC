...
Este capítulo apresenta os resultado obtido com a execução dos experimentos descritos no Capítulo XYZ ...
...
As tabelas que contém o detalhamento das execuções dos experimentos se encontram no Apêndice ZYX. A Tabela \ref{tab:medias_gi}
...
\clearpage
\subsection{Alteração de \textit{flags} de Otimização}
...
\begin{itemize}
 ...
 as \textit{flags} de otimização não tiveram uma grande impacto no tempo de otmização, como mostrado nos gráficos 
o tempo foi linear. 
...
\clearpage
\subsection{Alteração de \textit{flags} de processamento paralelo}
...
\subsection{Analise Geral}
 ...
 reduziram no maximo 12\% do tempo de compilação.
    ...
    Para a alteração de \textit{flags} no processo de compilação a \textit{flags} \texttt{-Ofast} foi a mais eficiente,
