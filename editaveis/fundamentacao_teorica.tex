...
lógica \cite[pag.3]{ref3}. Este arquivo é chamado de código fonte.
...
código de máquina \cite[pag.2]{ref4}.
...
equivalente uma outra linguagem, chamado de código objeto \cite[pag.2]{ref4}.
...
execute as instruções de máquina \cite[pag.17]{ref6}.
...
As linguagens que, em geral, são compiladas, lideram em performance em relação
 ...
 tomadas em tempo de compilação não são necessários no tempo de execução \cite[pag.2]{ref3}.
...
acesso, a fim de achar sua localização \cite[pag.167]{ref2}.
...
Para algumas linguagens existe o caso em que o compilador tem o papel
 de converter o código fonte em um \textit{byte code} \cite[pag.49]{ref9}. 
O \textit{byte code}
...
\begin{figure}[h]
    ...
    \caption{Fases da Compilação \cite[pag.26]{ref6}, com adaptações.}
    \label{fig01}
\end{figure}
...
analise semântica e geração de código intermediário \cite[pag.36]{ref6}.
...
código alvo, otimização de código  para o alvo específico \cite[pag.36]{ref6}.
...
sequência para instruções de máquina que realizarão as operações \cite[pag.10]{ref3}.
...
uma tarefa trabalhosa \cite[pag.5]{ref6}. 
...
usando notação hexadecimal para a representação dos bits, conforme apresentado
 no Código \ref{codigo_02} \cite[pag.5]{ref6}.
...
representa repetição e assim por diante) \cite[pag.5]{ref6}.

O mesmo programa mostrado no Código \ref{codigo_02} pode ser representado 
em assembly, conforme ilustrado no Código \ref{codigo_03} \cite[pag.5]{ref6}. 
...
correspondentes da linguagem de máquina (também chamado de opcode) \cite[pag.6]{ref6}.
...
(no inglês chamada de  \textit{include guards} ou \textit{guard conditions}),
 ...
 de inclusão, as quais serão citadas adiante \cite[pag.80]{ref42}.
...
qualquer informação que que esteja entre as diretivas \texttt{\#ifndef}  e \texttt{\#endif};
 ...
 que seguem a diretiva serão incluídas no arquivo resultante \cite[pag.80]{ref42}.
...
A declaração incompleta (\textit{forward declaration}) de uma classe somente podem
 ...
 de armazenamento para estes tipos de variáveis \cite[pag.111]{ref43}.
...
Na linguagem C++ há uma separação entre  a implementação e a interface de
 classe \cite[pag.140]{ref44}. Na maioria dos casos, a definição de uma classe possui
...
Definir uma classe com detalhes de implementação em sua definição não é uma
 ...
 \texttt{\#include}, gerando dependência entre arquivos \cite[pag.140]{ref44}.
...
criação do alvo. Este campo é opcional, pois podem existir instruções que não
 ...
 dos códigos do projeto) \cite[pag.1]{ref45}.
...
Este bloco pode conter mais de um comando em uma linha, ou  vários comandos em
 linhas separadas \cite[pag.1]{ref45}.

    \item Regras Implícitas: \
 conjunto de regras que informam quando e como refazer uma classe de arquivos
 baseadas em seus nomes e padrões \cite[pag.11]{ref45}. Um exemplo de utilização de 
 padrões é apresentado no Código \ref{codigo_22} \cite[pag.19]{ref45}
