...
\item \textit{Extensões de linguagens}: são formas de conferir maior poder as linguagens
    ...
    Equel\footnote{\url{http://www.eecs.berkeley.edu/~wong/wong\_pubs/wong46.pdf}
...
o \textit{Flex} 
\footnote{\url{http://dinosaur.compilertools.net/} acessado dia 20/06/2015}, 
o \textit{Bison} 
\footnote{\url{http://www.gnu.org/software/bison/} acessdo dia 20/06/2015}
 e o \textit{JavaCC} 
\footnote{\url{https://javacc.java.net/} acessado dia 20/06/2015}
 \cite{ref22}.
...
Pragma once é uma diretiva de pré-processamento que possui o objetivo de
...
\begin{table}[h]
	...
	\midrule
	\texttt{\#include<bits/stringfwd.h>} & \texttt{std::string},

							               \texttt{std::wstring},\texttt{std::u16string}, 

								           \texttt{std::u32string};\\
	\midrule
	\texttt{\#include <iosfwd>}          & \texttt{std::filebuf}, \texttt{std::fstream},
                    					   \texttt{std::ifstream}, \texttt{std::iostream},
                    					   \texttt{std::istream}, \texttt{std::ofstream},
                    					   \texttt{std::ostream}, \texttt{std::ostringstream},
                    					   \texttt{std::stringbuf}, \texttt{std::streambuf},
                    					    entre outros;\\
	\midrule
	\texttt{\#include <bits/localefwd.h>} & \texttt{std:has\_facet},\texttt{std::isalnum},
								            \texttt{std::isalpha}, \texttt{std::iscntrl},
								            \texttt{std::isdigit},\texttt{std::isgraph},
								            \texttt{std::islower}, \texttt{std::isprint},
								            \texttt{std::ispunct},
								            \texttt{std::isspace},\texttt{std::issuper},
								             entre outros;\\
	\midrule
	\texttt{\#include <bits/algorithmfwd.h>} & \texttt{std::adjacent\_find},\texttt{std::any\_of},
									           \texttt{std::binary\_search}, 
									           \texttt{std::copy\_backward}, \texttt{std::copy\_if},
									           \texttt{std::count\_if}, \texttt{std::count},
									           \texttt{std::find\_end}, \texttt{std::find\_if},
									           entre outros.\\
	\bottomrule
	\end{tabular}
\end{table}
