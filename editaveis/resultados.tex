...
\begin{itemize}
    ...
    Com base nos dados coletados e o gráfico gerado é possível inferir:
    \begin{enumerate}
        \item Todos os métodos que incluem \textbf{pragma once} atigiram tempo de compilação maior que os métodos que não possuem, independente do sistema operacional.
        \item  Dos métodos que não possuem pragma once o método redundante foi o que atingiu o menor tempo de compilação.
        \item  Em sistema operacional Mac OS Yosemite o tempo de compilação entre os métodos possui a menor variação.
        \item  Em ambiente Windows 7 o tempo de compilação dos métodos é entre 3 e 4 vezes maior que nos sistemas. 
    \end{enumerate}
\end{itemize}
...
\begin{itemize}
    \item Analise
Depois da coleta de dados e fazendo simples calculos é possível inferir::

    \begin{enumerate}
       \item Em projetos a utilização do pragma once não possui interferência
 ...
 do pragma once no projeto aseprite resultou em uma redução de 5\% nos
 melhores casos. 
        \item Como a aplicação do método forward declaration foi feita em projetos 
com tempo de compilação baixo, a diferença foi pouco perceptivel,
 com um valor maximo de 5\% na redução, no entanto nestes casos sempre foi reduzida. 
        \item A aplicação do método pimpl não possui uma interferência direta
 ...
 um projeto, caso os objetos intermediários da compilação não sejam apagados.
    \end{enumerate}
\end{itemize}
...
\begin{itemize}
    ...
    \item Analise
        ...
        Com a coleta de dados e a analise dos graficos é possivel inferir:

        \begin{enumerate}
            \item Analisando os projetos que utilizão qmake como gerador de makefile
 as flags de otimização não tiveram uma grande impacto no tempo de otmização, analisando os gráficos 
o tempo foi linear. 
            \item Analisando o projeto que possui como gerador de makefile como cmake a flag
 \textbf{-Ofast} obteve o menor tempo de redução, com um impacto significativo de aproximadamente de 
70\% a 75\% em todos os ambientes.
...
\end{table}
    ...
    Com a coleta de dados e a geração dos gráficos é possível inferir:
    
    \begin{enumerate}
        \item Independente do sistema operacional com a utilização da flags -j
 o tempo de compilação mais perceptível em projetos com o tempo de compilação
 com mais de 8 minutos, com redução de até 50\% com 2 threads.
 ...
 como exemplo o projeto aseprite ou o qcad do -j6 para o -j8 o tempo de compilação
...
\begin{itemize}
    ...
    Com os dados coletados e os gráficos gerados é possivel inferir:

    \begin{enumerate}
        \item Em todos os sistemas operacionais todos os projetos
 analisados sofrerão redução de mais de 65\% na redução do tempo de
 compilação dos projetos depois da primeira compilação.
        \item Analisando os graficos em todos os ambientes o primeiro
 caso na compilação utilizando a ferramenta ccache o tempo de compilação é o maior,
 ...
 compilado na primeira vez.
        \item Com a utilização da ferramenta gold o tempo de compilação
 ...
 redução do tempo de compilação dos projetos. 
        \item A utilização da ferramenta gold teve maior impacto no projeto aseprite, que utiliza
...
\subsection{Analise Geral}
    Após a analise de todos os dados coletados pode-se perceber dependendo do sistema operacional o
